...
策略后台按任务流和资产流拆分:默认进入“待复盘工单”,处理 Agent 无法确认或高风险的问题;切换到“策略资产库”后,按专家教学、专家纠错、工单复盘三个来源管理已沉淀策略。
...
本项目不提交 `.env` 和 SQLite 运行库。首次启动时会自动创建 `data_odm_demo.db` 并写入脱敏 ODM 示例数据;如需接入真实模型,请复制 `.env.example` 为 `.env` 后填写自己的模型配置。默认 mock 模式无需 API Key。
