...
顶部一级导航分为“策略验证 / 策略样本库 / 策略后台”。策略样本库是策略资产的唯一正式入口,页面内展示策略样本总数以及专家教学、专家纠错、工单复盘三个来源数量;策略后台只处理待复盘工单、排查策略总纲、完整系统提示词和 AI 业务设定。

模块边界可以这样理解:

```text
专家教学模式:专家主动录入一条新的问题排查经验。
...
```text
专家教学是主动录入,策略后台是处理待办,策略样本库是管理成品,策略验证是测试效果。
```
